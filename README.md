...
Na primeira etapa configuramos a toolchain. Pois assim será possível fazer o cross-compile para buildar o u-boot ou linux
Para baixar e instalar siga os seguintes comandos em seu terminal:
...
```
Agora iniciamos com o u-boot. 
Criamos uma pasta para ele chamada workdir e fizemos o git clone do repositório como segue:
...
```
Observe que o branch utilizado é o 2016.11-toradex, pois é o branch para a nossa placa VF50
...
```
Observe que a configuração colibri_vf_defconfig é específica para a nossa placa. Para maiores informações é possível acessar o tutorial da toradex com mais detalhes. O link está no final do Readme.
...
```
Atenção para a branch toradex_vf_4.4 que precisa ser especificamente essa para a nossa placa.
...
Segue uma imagem com esse trecho do código, para que seja possível ver como deverá ficar:
...
$ make vf500-colibri-eval-v3.dtb
```
Agora, basta trocar a dtb no módulo para que o CAN possa ser acessível no módulo.

# Testando o CAN

Para a comunicação CAN é necessária a instalação do CANopen, sendo necessário rodar em pelo menos 4 terminais diferentes em nosso projeto. Cada terminal com uma função. O terminal 1 para a função de comuniação, o terminal 2 para a criação do nó master, o terminal 3 para o nó servo e o terminal 4 para a transferência de comandos via cocomm. A seguir iremos testar o CAN de maneira simples.

Primeiro, testamos a CAN. Para realizar o teste, utilizaremos o tutorial da Lely CANopen. A pasta do git dele se encontra em nosso repositório e o link do tutorial está no final deste REadme. 
...
Agora, com as ferramentas em mãos, vamos fazer os testes a partir da ferramenta CANopen control. E então criaremos o master e o slave para a comunicação do CAN bus virtual. Conforme é mostrado nos comandos a seguir:
...
Para adicionar o slave, nós baixamos o arquivo cmd-slave.dcf do seguinte link:
...
A partir disso configuramos a produção de batimentos, conforme o tutorial da Lely sugere. Por meio da requisição de SDO, a qual em seguida aparece no candump:
...
Para a devida configuração é necessário identificar e alterar o id do nó. 
...
Para que a transmissão do PDO seja sincrona, precisamos escrever o período entre mensagens para o objeto (micro segundos). Para que os PDOs sejam enviados após o SYNC, precisamos ir no terminal do slave e configurar o TPDO. Após isso, o slave poderá criar o primeiro TPDO. Conforme segue na imagem:
...
![TESTE](https://user-images.githubusercontent.com/62487469/131559778-b1cf89ad-8059-45da-acf3-18a2849a6fc2.png)
...
> Habilitação do CAN na toradex Colibri VFxx:
> https://developer.toradex.com/knowledge-base/can-linux#Kernel_Support_Colibri_VFxx

> Criação da Build do kernel e onde conseguimos os arquivos do git toradex:
